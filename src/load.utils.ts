import { fail } from "k6";
import { assignClassToUserOrFail, createClassAndGetIdOrFail } from "./class.utils.js";
...
function createClasses(structureModel: Structure, classes: Set<string>, userModelBySpecId: Map<number, UserInfo>, users: LoadUser[]) {
    ...
    for(let user of users) {
        if(user.class) {
            for(let className of user.class) {
                ...
                if(!classId) {
                    fail("Class " + className + " not found for user " + user.firstName + " " + user.lastName);
                }
